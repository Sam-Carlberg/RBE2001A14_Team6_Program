...
int ArisGame(){
  ...
  case REVERSETOLINE1:
    if(getMovementSignal()){
      if(reset==0 && timeLocal > .25*WAIT_TIME && dirCount(storageTarget, BACKWARD)){
        reset=1;
        bounceNumCrosses = 0;
        ArisGameState++;
        moveDirection(0, STOP);
  ...
  case TURN1:
    if(getMovementSignal()){
      if(reset==0 && timeLocal > .75*WAIT_TIME && (turnAround(TURN_WAIT_TIME, getDirToStorage()) || timeLocal> 2.5*WAIT_TIME)){
        reset=1;
        ArisGameState++;
        moveDirection(0, STOP);
  ...
  case  TURN2:
    if(getMovementSignal()){
      if(reset==0 && timeLocal > .75*WAIT_TIME && (turnAround(WAIT_TIME+ TURN_WAIT_TIME/3, getDirToSupply()) || timeLocal>= 5.5*WAIT_TIME)){
  ...
  case TURN3:
    if(getMovementSignal()){
      if(reset==0 && timeLocal > .75*WAIT_TIME && (turnAround(WAIT_TIME+ TURN_WAIT_TIME/6, getDirToSupply())|| timeLocal > 2*WAIT_TIME)){
        intensity = HIGH_INTENSITY;
        reset=1;
        ArisGameState++;
        moveDirection(0, STOP);
  ...
  case TURN4: 
    if(getMovementSignal()){
      if(reset==0 && timeLocal > .75*WAIT_TIME && turnAround(WAIT_TIME+ TURN_WAIT_TIME/3, getDirToStorage())){
        reset=1;
        ArisGameState++;
        moveDirection(0, STOP);
      }
